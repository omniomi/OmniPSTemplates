...
#
# Clean, Build, Sign, BuildHelp, Analyze, Test and Publish.
#
# The default task is Build.  This task copies the appropriate files from the
# $SrcRootDir under the $OutDir.  Later, other tasks such as Sign will further
# modify the contents of $OutDir and add new files.
#
...
#
# The Publish task uses the Publish-Script command to publish
...
Task CoreStageFiles -requiredVariables ScriptOutDir, SrcRootDir {
    if (!(Test-Path -LiteralPath $ScriptOutDir)) {
        New-Item $ScriptOutDir -ItemType Directory -Verbose:$VerbosePreference > $null
    }
    else {
        Write-Verbose "$($psake.context.currentTaskName) - directory already exists '$ScriptOutDir'."
    }
...
Task Analyze -depends StageFiles `
             -requiredVariables ScriptOutDir, ScriptAnalysisEnabled, ScriptAnalysisFailBuildOnSeverityLevel, ScriptAnalyzerSettingsPath {
    ...
    $analysisResult = Invoke-ScriptAnalyzer -Path $ScriptOutDir -Settings $ScriptAnalyzerSettingsPath -Recurse -Verbose:$VerbosePreference
